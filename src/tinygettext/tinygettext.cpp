...
namespace tinygettext
{
    ...
    {
        return (n % 10 == 1 && n % 100 != 11 ? 0 : n != 0 ? 1
                                                          : 2);
    }
    ...
    {
        return (n % 10 == 1 && n % 100 != 11 ? 0 : n % 10 >= 2 && (n % 100 < 10 || n % 100 >= 20) ? 1
                                                                                                  : 2);
    }
    int plural3_1(int n)
    {
    ...
    {
        return (n == 1) ? 0 : (n >= 2 && n <= 4) ? 1
                                                 : 2;
    }
    int plural3_pl(int n)
    {
        return (n == 1 ? 0 : n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 10 || n % 100 >= 20) ? 1
                                                                                           : 2);
    }
    int plural3_sl(int n)
    {
        return (n % 100 == 1 ? 0 : n % 100 == 2               ? 1
                               : n % 100 == 3 || n % 100 == 4 ? 2
                                                              : 3);
    }
    ...
    DictionaryManager::get_dictionary(const std::string &spec)
    {
        ...
        else // Dictionary for languages lang isn't loaded, so we load it
        {
            ...
            {
                for (const auto &entry : std::filesystem::directory_iterator(p->c_str()))
                {
                    ...
                    if (load_from_file != "")
                    {
                        std::string pofile = *p + "/" + load_from_file;
                        try
                        {
                            std::ifstream in(pofile);
    ...
    DictionaryManager::add_directory(const std::string &pathname)
    {
        ...
        search_path.push_back("/data/" + pathname);
        set_language(language);
    }
    ...
    public:
            ...
            : dict(dict_)
        {
            ...
            char c = in.get();
            if (c == (char)0xef)
            { // skip UTF-8 intro that some texteditors produce
                in.get();
                in.get();
            }
            else
            ...
            while ((c = getchar(in)) != EOF)
            {
                ...
                case READ_CONTENT:
                    while ((c = getchar(in)) != EOF)
                    {
                        ...
                        { // Read something that may be a keyword
                            in.unget();
                            state = READ_KEYWORD;
                            add_token(token);
                            token = Token();
