...
void Construction::neighborize()
{
    ...
    if (!(flags & (FLAG_IS_GHOST)) && (constructionGroup->group != GROUP_FIRE) && (constructionGroup->group != GROUP_POWER_LINE))
    {
        if (flags & FLAG_IS_TRANSPORT) // search adjacent tiles only
        {
            ...
            for (unsigned short edge = 0; edge < size; ++edge)
            {
                ...
                if (cst && cst != cst1 && !(cst->flags & (FLAG_IS_GHOST)) && (cst->constructionGroup->group != GROUP_FIRE) && (cst->constructionGroup->group != GROUP_POWER_LINE))
                {
                    link_to(cst1 = cst);
                }
                cst = world(x + edge, y - 1)->reportingConstruction;
                if (cst && cst != cst2 && !(cst->flags & (FLAG_IS_GHOST)) && (cst->constructionGroup->group != GROUP_FIRE) && (cst->constructionGroup->group != GROUP_POWER_LINE))
                {
                    link_to(cst2 = cst);
                }
                cst = world(x + size, y + edge)->reportingConstruction;
                if (cst && cst != cst3 && !(cst->flags & (FLAG_IS_GHOST)) && (cst->constructionGroup->group != GROUP_FIRE) && (cst->constructionGroup->group != GROUP_POWER_LINE))
                {
                    link_to(cst3 = cst);
                }
                cst = world(x + edge, y + size)->reportingConstruction;
                if (cst && cst != cst4 && !(cst->flags & (FLAG_IS_GHOST)) && (cst->constructionGroup->group != GROUP_FIRE) && (cst->constructionGroup->group != GROUP_POWER_LINE))
                {
                    link_to(cst4 = cst);
                }
            }
            ...
            for (int yy = ys; yy < ye; ++yy)
            {
                for (int xx = xs; xx < xe; ++xx)
                {
                    ...
                    if (world(xx, yy)->construction) // be unique
                    {
                        ...
                        if ((cst->constructionGroup->group == GROUP_FIRE) || (cst->constructionGroup->group == GROUP_POWER_LINE))
                        {
                            continue;
                        }
...
void Construction::link_to(Construction *other)
{
    ...
    */
    ...
    for (stuff_ID = STUFF_INIT; !useful && stuff_ID < STUFF_COUNT; stuff_ID++)
    {
        ...
        if (!rule.maxload || !other_rule.maxload)
            continue;
...
int Construction::countPowercables(int mask)
{
    ...
    for (unsigned short i = 0; i < size; ++i)
    {
        ...
        if ((mask & 2) && world(x + size, y + i)->flags & FLAG_POWER_CABLES_90)
        {
            ++count;
        }
...
int Construction::tellstuff(Commodity stuff_ID, int center_ratio) // called by Minimap and connecttransport
{
    ...
    if (rule.maxload)
    {
        ...
        if ((center_ratio < 0) || (loc_ratio > center_ratio ? rule.give : rule.take))
        {
            return (loc_ratio);
        }
    }
...
void Construction::trade()
{
    ...
    for (stuff_ID = STUFF_INIT; stuff_ID < STUFF_COUNT; stuff_ID++)
    {
        ...
        for (unsigned int i = 0; i < lvls.size(); ++i)
        {
            ...
            else if (!(((long)lvlsi * center_cap > (long)center_lvl * capsi) ? (center_rule.take && pearrule.give) : (center_rule.give && pearrule.take)))
            {
                continue;
            }
            lvl += lvlsi;
        ...
        {
            unsigned short transportGroup = world(x, y)->getTransportGroup();

            transport->trafficCount[stuff_ID] = (9 * transport->trafficCount[stuff_ID] + max_traffic) / 10;
            if (getConfig()->carsEnabled && 100 * max_traffic * TRANSPORT_RATE / TRANSPORT_QUANTA > 2 &&  
               (transportGroup == GROUP_ROAD || transportGroup == GROUP_TRACK))
            {
             ...
             /*   if (lincitySpeed == MED_TIME_FOR_YEAR) // compensate for overall animation
                ...
                if (transport->canPlaceVehicle()) 
                {
                    VehicleStrategy strategy = (flow > 0) ? VEHICLE_STRATEGY_MAXIMIZE : VEHICLE_STRATEGY_MINIMIZE;

                    switch (stuff_ID)
                    {
                    case STUFF_FOOD:
                        if ((rand() % FOOD_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
                    case STUFF_LABOR:
                        if ((rand() % LABOR_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
                    case STUFF_COAL:
                        if ((rand() % COAL_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
                    case STUFF_GOODS:
                        if ((rand() % GOODS_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
                    case STUFF_ORE:
                        if ((rand() % ORE_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
                    case STUFF_STEEL:
                        if ((rand() % STEEL_TRAFFIC_RATE) < (yield + 1) / 2)
                        {
                            new Vehicle(x, y, stuff_ID, strategy);
                        }
                        break;
        ...
        else if (powerline) // Special for powerlines
        {
            ...
            {
                if ((powerline->anim_counter == 0) && !(neighbors[i]->constructionGroup->group == GROUP_POWER_LINE) && neighbors[i]->constructionGroup->commodityRuleCount[stuff_ID].give && (neighbors[i]->commodityCount[stuff_ID] > 0))
                {
...
int Construction::equilibrate_stuff(int *rem_lvl, CommodityRule rem_rule, int ratio, Commodity stuff_ID)
{
    ...
    if (!(flags & FLAG_EVACUATE))
    {
        ...
        if (flow > 0 ? !(loc_rule.take && rem_rule.give) : !(loc_rule.give && rem_rule.take))
        ...
        else if (flow < 0)
        {
            ...
            if (-flow > (rem_cap - *rem_lvl))
            {
                ...
                if (flow > 0)
                    flow = 0; // the other construction is evacuating
            }
        }
        // std::cout.flush();
        if (!(flags & FLAG_IS_TRANSPORT) && (flow > 0) && (constructionGroup->group != GROUP_MARKET))
...
{
    return static_cast<int>(cost * (1.0f + (cost_mul * tech_level) / static_cast<float>(MAX_TECH_LEVEL)));
}
