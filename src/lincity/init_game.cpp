...
void create_new_city(int *originx, int *originy, city_settings *city, int old_setup_ground, int climate)
{
    ...
    if (city != NULL)
    { // Only if we are not reconstructing from seed
        clear_game();
...
static void new_setup_river_ground(void)
{
    ...
    *f1(sz, sz) = (float)(rand() % Keco + rand() % Keco) * global_mountainity;
            ...
            for (m = 0; m < n; m++)
            {
                ...
                */
                ...
                if (!*f1(midx, midy))
                {
                    ...
                    float center = (left_top + left_down + right_top + right_down) / 4 + float((rand() % Keco - Keco / 2) * global_mountainity) * pow(fract, k);
                    *f1(midx, midy) = center;
                }
            }
        }
    ...
    // always skip (0,0) ensures brown background
    {
        i = index % len;
        j = index / len;
        if (((i == 0) && world(1, j)->is_river()) || (i == len - 1 && world(len - 2, j)->is_river()) || (j == 0 && world(1, j)->is_river()) || (j == len - 1 && world(i, len - 2)->is_river()))
        {
            set_river_tile(i, j);
        }
    }
...
static int overfill_lake(int start_x, int start_y) //, Shoreline *shore, int lake_id)
{
    ...
    *i1(start_x, start_y) = 1;
    ...
    {
        std::cout << std::endl
                  << "cancelled lake x, y = " << start_x << ", " << start_y << " alt : " << level << std::endl;
        return -1;
    }
...
static void setup_river2(int x, int y, int d, int alt, int mountain)
{
    ...
    for (j = 0; j < i; j++)
    {
        ...
        x += r;
        if ((world.is_inside(x + 2 * d, y) && !world(x + 2 * d, y)->is_bare()) || (world.is_inside(x + 3 * d, y) && !world(x + 3 * d, y)->is_bare()))
            return;
