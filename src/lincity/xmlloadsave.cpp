...
#ifdef DEBUG
            std::cout << std::endl
                      << "loadsave_version " << ldsv_version << std::endl;
#endif
        }
...
void XMLloadsave::loadTileTemplates()
{
    ...
    for (int i = 0; i < last_i; ++i)
    {
        ...
        while (!cur_template->reached_end())
        {
            ...
            if (xml_tag == "flags")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->flags, sizeof(cur_tile->flags));
            }
            else if (xml_tag == "air_pol")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->pollution, sizeof(cur_tile->pollution));
            }
            else if (xml_tag == "ore")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ore_reserve, sizeof(cur_tile->ore_reserve));
            }
            else if (xml_tag == "coal")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->coal_reserve, sizeof(cur_tile->coal_reserve));
            }
            // ground
            else if (xml_tag == "altitude")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.altitude, sizeof(cur_tile->ground.altitude));
            }
            else if (xml_tag == "ecotable")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.ecotable, sizeof(cur_tile->ground.ecotable));
            }
            else if (xml_tag == "wastes")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.wastes, sizeof(cur_tile->ground.wastes));
            }
            else if (xml_tag == "grd_pol")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.pollution, sizeof(cur_tile->ground.pollution));
            }
            else if (xml_tag == "water_alt")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.water_alt, sizeof(cur_tile->ground.water_alt));
            }
            else if (xml_tag == "water_pol")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.water_pol, sizeof(cur_tile->ground.water_pol));
            }
            else if (xml_tag == "water_wast")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.water_wast, sizeof(cur_tile->ground.water_wast));
            }
            else if (xml_tag == "water_next")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.water_next, sizeof(cur_tile->ground.water_next));
            }
            else if (xml_tag == "int1")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.int1, sizeof(cur_tile->ground.int1));
            }
            else if (xml_tag == "int2")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.int2, sizeof(cur_tile->ground.int2));
            }
            else if (xml_tag == "int3")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.int3, sizeof(cur_tile->ground.int3));
            }
            else if (xml_tag == "int4")
            {
                cm += gzread(gz_xml_file, (char *)&cur_tile->ground.int4, sizeof(cur_tile->ground.int4));
            }
            else
            {
...
void XMLloadsave::loadConstructionTemplates()
{
...
void XMLloadsave::loadGlobals()
{
    ...
    do
    {
        ...
        if ((r == 2) && (xml_val.length()))
        {
            ...
            globalCount++;
            if (xml_tag == "binary_mode")
            {
                binary_mode = (xml_val == "1") || (xml_val == "yes");
            }
            else if (xml_tag == "seed_compression")
            {
                seed_compression = (xml_val == "1") || (xml_val == "yes");
            }
            else if (xml_tag == "altered_tiles")
            {
                sscanf(xml_val.c_str(), "%d", &altered_tiles);
            }

            else if (xml_tag == "constructions")
            {
                sscanf(xml_val.c_str(), "%d", &totalConstructions);
            }
            else if (xml_tag == "given_scene")
            {
                strcpy(given_scene, xml_val.c_str());
            }
            else if (xml_tag == "global_aridity")
            {
                sscanf(xml_val.c_str(), "%d", &global_aridity);
            }
            else if (xml_tag == "global_mountainity")
            {
                sscanf(xml_val.c_str(), "%d", &global_mountainity);
            }
            else if (xml_tag == "world_side_len")
            {
                sscanf(xml_val.c_str(), "%d", &new_world_len);
                world.len(new_world_len);
            }
            else if (xml_tag == "world_id")
            {
                sscanf(xml_val.c_str(), "%d", &world_id);
                world.seed(world_id);
            }
            else if (xml_tag == "old_setup_ground")
            {
                sscanf(xml_val.c_str(), "%d", &world.old_setup_ground);
            }
            else if (xml_tag == "climate")
            {
                sscanf(xml_val.c_str(), "%d", &world.climate);
            }

            else if (xml_tag == "main_screen_originx")
            {
                sscanf(xml_val.c_str(), "%d", &main_screen_originx);
            }
            else if (xml_tag == "main_screen_originy")
            {
                sscanf(xml_val.c_str(), "%d", &main_screen_originy);
            }
            else if (xml_tag == "total_time")
            {
                sscanf(xml_val.c_str(), "%d", &total_time);
            }

            else if (xml_tag == "people_pool")
            {
                sscanf(xml_val.c_str(), "%d", &people_pool);
            }
            else if (xml_tag == "total_money")
            {
                sscanf(xml_val.c_str(), "%d", &total_money);
            }
            else if (xml_tag == "ly_income_tax")
            {
                sscanf(xml_val.c_str(), "%d", &ly_income_tax);
            }
            else if (xml_tag == "income_tax")
            {
                sscanf(xml_val.c_str(), "%d", &income_tax);
            }
            else if (xml_tag == "income_tax_rate")
            {
                sscanf(xml_val.c_str(), "%d", &income_tax_rate);
            }

            else if (xml_tag == "ly_interest")
            {
                sscanf(xml_val.c_str(), "%d", &ly_interest);
            }
            else if (xml_tag == "ly_coal_tax")
            {
                sscanf(xml_val.c_str(), "%d", &ly_coal_tax);
            }
            else if (xml_tag == "coal_tax")
            {
                sscanf(xml_val.c_str(), "%d", &coal_tax);
            }
            else if (xml_tag == "coal_tax_rate")
            {
                sscanf(xml_val.c_str(), "%d", &coal_tax_rate);
            }

            else if (xml_tag == "ly_unemployment_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_unemployment_cost);
            }
            else if (xml_tag == "unemployment_cost")
            {
                sscanf(xml_val.c_str(), "%d", &unemployment_cost);
            }
            else if (xml_tag == "dole_rate")
            {
                sscanf(xml_val.c_str(), "%d", &dole_rate);
            }

            else if (xml_tag == "ly_transport_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_transport_cost);
            }
            else if (xml_tag == "transport_cost")
            {
                sscanf(xml_val.c_str(), "%d", &transport_cost);
            }
            else if (xml_tag == "transport_cost_rate")
            {
                sscanf(xml_val.c_str(), "%d", &transport_cost_rate);
            }
            else if (xml_tag == "ly_goods_tax")
            {
                sscanf(xml_val.c_str(), "%d", &ly_goods_tax);
            }
            else if (xml_tag == "goods_tax")
            {
                sscanf(xml_val.c_str(), "%d", &goods_tax);
            }
            else if (xml_tag == "goods_tax_rate")
            {
                sscanf(xml_val.c_str(), "%d", &goods_tax_rate);
            }

            else if (xml_tag == "ly_export_tax")
            {
                sscanf(xml_val.c_str(), "%d", &ly_export_tax);
            }
            else if (xml_tag == "export_tax")
            {
                sscanf(xml_val.c_str(), "%d", &export_tax);
            }
            else if (xml_tag == "export_tax_rate")
            {
                sscanf(xml_val.c_str(), "%d", &export_tax_rate);
            }
            else if (xml_tag == "ly_import_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_import_cost);
            }
            else if (xml_tag == "import_cost")
            {
                sscanf(xml_val.c_str(), "%d", &import_cost);
            }
            else if (xml_tag == "import_cost_rate")
            {
                sscanf(xml_val.c_str(), "%d", &import_cost_rate);
            }

            else if (xml_tag == "ly_university_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_university_cost);
            }
            else if (xml_tag == "university_cost")
            {
                sscanf(xml_val.c_str(), "%d", &university_cost);
            }
            else if (xml_tag == "ly_recycle_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_recycle_cost);
            }
            else if (xml_tag == "recycle_cost")
            {
                sscanf(xml_val.c_str(), "%d", &recycle_cost);
            }
            else if (xml_tag == "ly_school_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_school_cost);
            }
            else if (xml_tag == "school_cost")
            {
                sscanf(xml_val.c_str(), "%d", &school_cost);
            }

            else if (xml_tag == "ly_health_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_health_cost);
            }
            else if (xml_tag == "health_cost")
            {
                sscanf(xml_val.c_str(), "%d", &health_cost);
            }
            else if (xml_tag == "ly_deaths_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_deaths_cost);
            }
            else if (xml_tag == "deaths_cost")
            {
                sscanf(xml_val.c_str(), "%d", &deaths_cost);
            }
            else if (xml_tag == "ly_rocket_pad_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_rocket_pad_cost);
            }
            else if (xml_tag == "rocket_pad_cost")
            {
                sscanf(xml_val.c_str(), "%d", &rocket_pad_cost);
            }

            else if (xml_tag == "ly_windmill_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_windmill_cost);
            }
            else if (xml_tag == "windmill_cost")
            {
                sscanf(xml_val.c_str(), "%d", &windmill_cost);
            }
            else if (xml_tag == "ly_fire_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_fire_cost);
            }
            else if (xml_tag == "fire_cost")
            {
                sscanf(xml_val.c_str(), "%d", &fire_cost);
            }
            else if (xml_tag == "ly_cricket_cost")
            {
                sscanf(xml_val.c_str(), "%d", &ly_cricket_cost);
            }
            else if (xml_tag == "cricket_cost")
            {
                sscanf(xml_val.c_str(), "%d", &cricket_cost);
            }

            else if (xml_tag == "tech_level")
            {
                sscanf(xml_val.c_str(), "%d", &tech_level);
            }
            else if (xml_tag == "highest_tech_level")
            {
                sscanf(xml_val.c_str(), "%d", &highest_tech_level);
            }
            else if (xml_tag == "tpopulation")
            {
                sscanf(xml_val.c_str(), "%d", &tpopulation);
            }
            else if (xml_tag == "thousing")
            {
                sscanf(xml_val.c_str(), "%d", &thousing);
            }
            else if (xml_tag == "tstarving_population")
            {
                sscanf(xml_val.c_str(), "%d", &tstarving_population);
            }
            else if (xml_tag == "tunemployed_population")
            {
                sscanf(xml_val.c_str(), "%d", &tunemployed_population);
            }

            else if (xml_tag == "total_pollution")
            {
                sscanf(xml_val.c_str(), "%d", &total_pollution);
            }
            else if (xml_tag == "rockets_launched")
            {
                sscanf(xml_val.c_str(), "%d", &rockets_launched);
            }
            else if (xml_tag == "rockets_launched_success")
            {
                sscanf(xml_val.c_str(), "%d", &rockets_launched_success);
            }

            else if (xml_tag == "coal_survey_done")
            {
                sscanf(xml_val.c_str(), "%d", &coal_survey_done);
            }
            else if (xml_tag == "cheat_flag")
            {
                sscanf(xml_val.c_str(), "%d", &cheat_flag);
            }
            else if (xml_tag == "total_pollution_deaths")
            {
                sscanf(xml_val.c_str(), "%d", &total_pollution_deaths);
            }
            else if (xml_tag == "pollution_deaths_history")
            {
                sscanf(xml_val.c_str(), "%f", &pollution_deaths_history);
            }
            else if (xml_tag == "total_starve_deaths")
            {
                sscanf(xml_val.c_str(), "%d", &total_starve_deaths);
            }
            else if (xml_tag == "starve_deaths_history")
            {
                sscanf(xml_val.c_str(), "%f", &starve_deaths_history);
            }

            else if (xml_tag == "total_unemployed_years")
            {
                sscanf(xml_val.c_str(), "%d", &total_unemployed_years);
            }
            else if (xml_tag == "unemployed_history")
            {
                sscanf(xml_val.c_str(), "%f", &unemployed_history);
            }
            else if (xml_tag == "max_pop_ever")
            {
                sscanf(xml_val.c_str(), "%d", &max_pop_ever);
            }
            else if (xml_tag == "total_evacuated")
            {
                sscanf(xml_val.c_str(), "%d", &total_evacuated);
            }
            else if (xml_tag == "total_births")
            {
                sscanf(xml_val.c_str(), "%d", &total_births);
            }

            else if (xml_tag == "sust_dig_ore_coal_tip_flag")
            {
                sscanf(xml_val.c_str(), "%d", &sust_dig_ore_coal_tip_flag);
            }
            else if (xml_tag == "sust_dig_ore_coal_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_dig_ore_coal_count);
            }
            else if (xml_tag == "sust_port_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_port_count);
            }
            else if (xml_tag == "sust_old_money_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_money_count);
            }
            else if (xml_tag == "sust_old_population_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_population_count);
            }
            else if (xml_tag == "sust_old_tech_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_tech_count);
            }
            else if (xml_tag == "sust_fire_count")
            {
                sscanf(xml_val.c_str(), "%d", &sust_fire_count);
            }
            else if (xml_tag == "sust_old_money")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_money);
            }
            else if (xml_tag == "sust_port_flag")
            {
                sscanf(xml_val.c_str(), "%d", &sust_port_flag);
            }
            else if (xml_tag == "sust_old_population")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_population);
            }
            else if (xml_tag == "sust_old_tech")
            {
                sscanf(xml_val.c_str(), "%d", &sust_old_tech);
            }
            else if (xml_tag == "sustain_flag")
            {
                sscanf(xml_val.c_str(), "%d", &sustain_flag);
            }
    ...
    ly_other_cost = ly_university_cost + ly_recycle_cost + ly_deaths_cost + ly_health_cost + ly_rocket_pad_cost + ly_school_cost + ly_interest + ly_windmill_cost + ly_fire_cost + ly_cricket_cost;
    ...
    do
    {
        ...
        if (!prescan && inside_MapTile)
        {
            ...
            if ((r == 2) && (xml_val.length()))
            {
                ...
                if (xml_tag == "map_x")
                    ; // ignore map_x
                else if (xml_tag == "map_y")
                    ; // ignore map_y
                // mapTile
                else if (xml_tag == "flags")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->flags);
                }
                else if (xml_tag == "type")
                {
                    sscanf(xml_val.c_str(), "%hu", &cur_tile->type);
                }
                else if (xml_tag == "group")
                {
                    sscanf(xml_val.c_str(), "%hu", &cur_tile->group);
                }
                else if (xml_tag == "air_pol")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->pollution);
                }
                else if (xml_tag == "ore")
                {
                    sscanf(xml_val.c_str(), "%hu", &cur_tile->ore_reserve);
                }
                else if (xml_tag == "coal")
                {
                    sscanf(xml_val.c_str(), "%hu", &cur_tile->coal_reserve);
                }
                // ground
                else if (xml_tag == "altitude")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.altitude);
                }
                else if (xml_tag == "ecotable")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.ecotable);
                }
                else if (xml_tag == "wastes")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.wastes);
                }
                else if (xml_tag == "grd_pol")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.pollution);
                }
                else if (xml_tag == "water_alt")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.water_alt);
                }
                else if (xml_tag == "water_pol")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.water_pol);
                }
                else if (xml_tag == "water_wast")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.water_wast);
                }
                else if (xml_tag == "water_next")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.water_next);
                }
                else if (xml_tag == "int1")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.int1);
                }
                else if (xml_tag == "int2")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.int2);
                }
                else if (xml_tag == "int3")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.int3);
                }
                else if (xml_tag == "int4")
                {
                    sscanf(xml_val.c_str(), "%d", &cur_tile->ground.int4);
                }
                else
                {
...
void XMLloadsave::readPbar()
{
    ...
    do
    {
        ...
        if (r == 2)
        {
            ...
            {
                sscanf(xml_val.c_str(), "%d", &pbars[ID].oldtot);
                oldtot_ok = true;
            }
            else if (xml_tag == "diff" && (ID != -1))
            {
                sscanf(xml_val.c_str(), "%d", &pbars[ID].diff);
                diff_ok = true;
            }
            else if (xml_tag != "ID")
            {
...
int XMLloadsave::sliceXMLline()
{
    ...
    if ((position[0] < position[1]) && (position[1] == line.length() - 1))
    {
        xml_tag = line.substr(position[0] + 1, position[1] - position[0] - 1);
        xml_tag2.clear();
        xml_val.clear();
