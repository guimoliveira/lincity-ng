...
void ConstructionCount::add_construction(Construction *construction)
{

    while (free_slot < constructionVector.size() && constructionVector[free_slot])
    {
        free_slot++;
    }
...
void ConstructionCount::remove_construction(Construction *construction)
{
    free_slot = 0;
    while (free_slot < constructionVector.size() && constructionVector[free_slot] != construction)
    {
        free_slot++;
    }
...
void ConstructionCount::shuffle()
{
    permutator->shuffle();
}
...
void ConstructionCount::update_permutator()
{
    if (permutator)
    {
        delete permutator;
...
int ConstructionCount::size()
{
    return constructionVector.size();
}

void ConstructionCount::size(int new_len)
{
    constructionVector.resize(new_len, NULL);
    update_permutator();
}

int ConstructionCount::count()
{
    int n = 0;
...
bool ConstructionCount::earlier(Construction *a, Construction *b)
{
    // sort works until all tests are false
    return (a && b) ? ((a->constructionGroup->group == b->constructionGroup->group) ? (a->ID < b->ID) : (a->constructionGroup->group < b->constructionGroup->group)) : false;
}

// FIXME Only for use in debugging
void ConstructionCount::reset()
{
