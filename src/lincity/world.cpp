...
ConstructionGroup *MapTile::getTileConstructionGroup()
{
    ...
    {
    case GROUP_BARE:
        return &bareConstructionGroup;
        break;
    case GROUP_DESERT:
        return &desertConstructionGroup;
        break;
    case GROUP_WATER:
        return &waterConstructionGroup;
        break;
    case GROUP_TREE:
        return &treeConstructionGroup;
        break;
    case GROUP_TREE2:
        return &tree2ConstructionGroup;
        break;
    case GROUP_TREE3:
        return &tree3ConstructionGroup;
        break;
    default:
...
{
    return (reportingConstruction && ((reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_LL) || (reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_ML) || (reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_HL) || (reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_LH) || (reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_MH) || (reportingConstruction->constructionGroup->group == GROUP_RESIDENCE_HH)));
}

void MapTile::writeTemplate()
{
    std::string xml_tag;
    if ((!binary_mode && xml_template_libary.count("tile") == 0) || (binary_mode && bin_template_libary.count(GROUP_DESERT) == 0))
    {
...
void MapTile::saveMembers(std::ostream *os)
{
    ...
    size_t cm = 0;
    std::string xml_tag;
    if ((!binary_mode && xml_template_libary.count("tile") == 0) || (binary_mode && bin_template_libary.count(head) == 0))
    {
        writeTemplate();
    }
    XMLTemplate *xml_tmp;
    ...
    if (!binary_mode)
    {
        *os << "<tile>";
        ...
        while (!xml_tmp->reached_end())
        {
            ...
            {
                *os << ground.water_wast;
            }
            else if (xml_tag == "water_next")
            {
                *os << ground.water_next;
            }
    ...
    else
    {
        ...
        while (!xml_tmp->reached_end())
        {
            ...
            {
                os->write((char *)&pollution, sizeof(pollution));
                cm += sizeof(pollution);
            }
            else if (xml_tag == "ore")
            {
                os->write((char *)&ore_reserve, sizeof(ore_reserve));
                cm += sizeof(ore_reserve);
            }
            else if (xml_tag == "coal")
            {
                os->write((char *)&coal_reserve, sizeof(coal_reserve));
                cm += sizeof(coal_reserve);
            }
            // ground
            else if (xml_tag == "altitude")
            {
                os->write((char *)&ground.altitude, sizeof(ground.altitude));
                cm += sizeof(ground.altitude);
            }
            else if (xml_tag == "ecotable")
            {
                os->write((char *)&ground.ecotable, sizeof(ground.ecotable));
                cm += sizeof(ground.ecotable);
            }
            else if (xml_tag == "wastes")
            {
                os->write((char *)&ground.wastes, sizeof(ground.wastes));
                cm += sizeof(ground.wastes);
            }
            else if (xml_tag == "grd_pol")
            {
                os->write((char *)&ground.pollution, sizeof(ground.pollution));
                cm += sizeof(ground.pollution);
            }
            else if (xml_tag == "water_alt")
            {
                os->write((char *)&ground.water_alt, sizeof(ground.water_alt));
                cm += sizeof(ground.water_alt);
            }
            else if (xml_tag == "water_pol")
            {
                os->write((char *)&ground.water_pol, sizeof(ground.water_pol));
                cm += sizeof(ground.water_pol);
            }
            else if (xml_tag == "water_wast")
            {
                os->write((char *)&ground.water_wast, sizeof(ground.water_wast));
                cm += sizeof(ground.water_wast);
            }
            else if (xml_tag == "water_next")
            {
                os->write((char *)&ground.water_next, sizeof(ground.water_next));
                cm += sizeof(ground.water_next);
            }
            else if (xml_tag == "int1")
            {
                os->write((char *)&ground.int1, sizeof(ground.int1));
                cm += sizeof(ground.int1);
            }
            else if (xml_tag == "int2")
            {
                os->write((char *)&ground.int2, sizeof(ground.int2));
                cm += sizeof(ground.int2);
            }
            else if (xml_tag == "int3")
            {
                os->write((char *)&ground.int3, sizeof(ground.int3));
                cm += sizeof(ground.int3);
            }
            else if (xml_tag == "int4")
            {
                os->write((char *)&ground.int4, sizeof(ground.int4));
...
bool World::checkEdgeMin(int x, int y)
{
    ...
    if (x == 1 || x == side_len - 2)
    {
        return alt < maptile[x + 1 + y * side_len].ground.altitude && alt < maptile[x - 1 + y * side_len].ground.altitude;
    }
    else if (y == 1 || y == side_len - 2)
    {
        return alt < maptile[x + (y + 1) * side_len].ground.altitude && alt < maptile[x + (y - 1) * side_len].ground.altitude;
    }
    else
        return false;
}
