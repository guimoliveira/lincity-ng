...
void MiniMap::toggleStuffID(int step)
{
...
void MiniMap::parse(XmlReader &reader)
{
    // parse attributes...
    ...
    while (iter.next())
    {
        ...
        {
            if (sscanf(value.c_str(), "%d", &border) != 1)
            {
                std::stringstream msg;
        ...
        {
            if (sscanf(value.c_str(), "%d", &tilesize) != 1)
            {
                std::stringstream msg;
...
void MiniMap::switchButton(CheckButton *button, int mousebutton)
{
    if (!alreadyAttached)
    {
        return;
    ...
    else if (buttonName == "SwitchPBar")
    {
        ...
        if ((active_button_name == "SwitchPBar") || (active_button_name == "SwitchPBar2nd"))
        {
...
void MiniMap::switchView(const std::string &viewname)
{
    SwitchComponent *switchComponent = getSwitchComponent(*(findRoot(this)), "MiniMapSwitch");
...
void MiniMap::switchMapViewButton(const std::string &buttonName)
{
    ...
    else if (buttonName == "MapViewPower" || buttonName == "MapViewHealth" || buttonName == "MapViewFire" || buttonName == "MapViewSport")
    {
    ...
    if (switchName != "")
    {
        SwitchComponent *switchComponent = getSwitchComponent(*(findRoot(this)), switchName);
...
void MiniMap::speedButtonClicked(CheckButton *button, int)
{
    Component *root = findRoot(this);
    std::string name = button->getName();
    int i;
...
void MiniMap::zoomInButtonClicked(Button *)
{
    getGameView()->zoomIn();
}

void MiniMap::zoomOutButtonClicked(Button *)
{
    getGameView()->zoomOut();
}

void MiniMap::scrollPageDownButtonClicked(Button *)
{
    scrollPageDown(true);
}

void MiniMap::scrollPageUpButtonClicked(Button *)
{
    scrollPageDown(false);
}

void MiniMap::scrollPageDown(bool down)
{
...
// #define DEBUG_ALTITUDE
    ...
    case UB40:
    {
        ...
        int job_level = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(STUFF_LABOR, -1) : -1;
    ...
    case STARVE:
    {
        int food_level = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(STUFF_FOOD, -1) : 1;
        int water_level = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(STUFF_WATER, -1) : 1;
    ...
    case POWER:
    {
        ...
        int lovolt_level = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(STUFF_LOVOLT, -1) : -1;
        int hivolt_level = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(STUFF_HIVOLT, -1) : -1;
    ...
    case COMMODITIES:
    {
        /*
                    if (conflags & FLAG_IS_TRANSPORT)
                        return makeGrey(getColorNormal(xx,yy));
        */
        float loc_lvl = world(xx, yy)->reportingConstruction ? world(xx, yy)->reportingConstruction->tellstuff(stuff_ID, -1) : -1;
...
void MiniMap::event(const Event &event)
{
    ...
    if (event.type == Event::MOUSEMOTION || event.type == Event::MOUSEBUTTONDOWN)
    {
        if (!event.inside)
        {
            inside = false;
        ...
        if (!inside)
        { // mouse just enterd the minimap, show current mapmode
            getGameView()->setMapMode(mMode);
            inside = true;
        }
        if (event.mousebuttonstate & SDL_BUTTON(SDL_BUTTON_LEFT) || event.type == Event::MOUSEBUTTONDOWN)
        {
            MapPoint tile(
