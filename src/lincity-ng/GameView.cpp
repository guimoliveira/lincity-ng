...
{
    std::string nfilename = std::string("/data/images/tiles/") + filename;
...
SDL_Surface *GameView::readImage(const std::string &filename)
{
    ...
    currentImage = IMG_Load(nfilename.c_str());
    if (!currentImage)
    {
...
{
    std::string xmlfile = "/data/images/tiles/images.xml";
        ...
        if (reader.getNodeType() == XML_READER_TYPE_ELEMENT)
        {
            ...
            if (element == "image")
            {
                ...
                while (iter.next())
                {
                    ...
                    {
                        if (sscanf(value.c_str(), "%i", &xmlX) != 1)
                        {
                    ...
                    {
                        if (sscanf(value.c_str(), "%i", &xmlY) != 1)
                        {
...
void GameView::event(const Event &event)
{
    ...
    case Event::MOUSEMOTION:
    {
        ...
        if (roadDragging && ((userOperation->action == UserOperation::ACTION_BULLDOZE)) && !areaBulldoze)
        {
            if (tile != startRoad)
            {
    ...
    case Event::MOUSEBUTTONDOWN:
    {
        ...
        if (event.mousebutton == SDL_BUTTON_LEFT)
        {
            ...
            if (isTouchDevice() && userOperation->action == UserOperation::ACTION_QUERY)
            {
                dragging = false;
                rightButtonDown = true;
                setPanningCursor();
    ...
    case Event::MOUSEBUTTONUP:
        ...
        {
            resetLastMessage();
            if (dragging && isTouchDevice() && userOperation->action == UserOperation::ACTION_QUERY)
            {
                dragging = false;
                rightButtonDown = false;
                setDefaultCursor();
    ...
    case Event::KEYUP:
        ...
        */
        ...
        if (event.keysym.scancode == SDL_SCANCODE_RETURN || event.keysym.scancode == SDL_SCANCODE_RETURN2)
        {
            resetZoom();
            break;
        }
...
MapPoint GameView::getTile(const Vector2 &p)
{
    ...
    float x = (point.x - virtualScreenWidth / 2) / tileWidth + point.y / tileHeight;
    tile.x = (int)floorf(x);
...
{
    if (!buttonsConnected)
        connectButtons(); // has to be done here,
        ...
        for (size_t i = 0; i < it->second->graphicsInfoVector.size(); ++i)
        {
            if (!it->second->graphicsInfoVector[i].texture)
            {
                if (it->second->graphicsInfoVector[i].image)
                {
                    ...
                    if (it->second->graphicsInfoVector[i].texture)
                    {
                        it->second->graphicsInfoVector[i].image = 0;
                    } // Image was erased by texture_manager->create.
                    --remaining_images;
                }
            }
        }
...
void GameView::drawTile(Painter &painter, const MapPoint &tile)
{
    ...
    if ((tile.x == x) && (tile.y - size + 1 == y)) // Signs are tested
    {
        ...
        if (hideHigh && (cstgrp == &treeConstructionGroup || cstgrp == &tree2ConstructionGroup || cstgrp == &tree3ConstructionGroup))
        {
...
int GameView::buildCost(MapPoint tile)
{
    ...
    if (userOperation->action == UserOperation::ACTION_BUILD)
    {
        ...
        userOperation->constructionGroup->getCosts();
    }
